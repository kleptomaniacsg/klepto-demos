...
The standard `plan-comparison` template includes a small configuration map that
can tweak transformation behaviour.  The most common option is
`columnSpacing` which controls how many empty columns appear between each plan
for readability.  Starting with vX.Y we also support `valuesOnly` which tells
the server that the template already contains the benefit names in the first
column; the matrix generated by the transformer will then omit that column and
be injected under a different data key.

```yaml
# plan-comparison.yaml
templateId: plan-comparison
config:
  columnSpacing: 1      # number of spacer columns between plan columns
  # valuesOnly: false   # set to true if benefit column is pre-populated
sections:
...
A companion template `plan-comparison-values-only.yaml` demonstrates the
`valuesOnly` mode.  It maps a range starting in column **B** and expects the
matrix to be provided under `$.comparisonMatrixValues`:

```yaml
# plan-comparison-values-only.yaml
